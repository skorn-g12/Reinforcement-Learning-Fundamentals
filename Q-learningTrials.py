...
def new_strategy_func(s, policy):
    action = max(policy_followed[s])
    if len(policy_followed[s]) > 100:
        if (policy_followed[s].count(action) / len(policy_followed[s])) > 0.7:
            return action
        else:
            return epsilon_greedy(s, policy)
    else:
        return epsilon_greedy(s, policy)
    ...
    while iter < 10000:  # Convergence loop
        ...
        while True:  # Game loop
            ...
            if s2 not in terminal_states:
                ...
                a_new = new_strategy_func(s2, policy)
                mod_Q = mu * Q[s2, a_max] + ((1 - mu) ** step) * Q[s2, a_new]
                # mod_Q = Q[s2, a_max]
                Q[s, a] = Q[s, a] + alpha * ((r + gamma * mod_Q) - Q[s, a])
            else:
        ...
        if iter % 100 == 0:
            ...
            for s in permissible_initial_states:
                ...
                policy_mod[s] = possible_actions[max_idx]
            print("iter", iter)
            print("policy:", policy_mod)
