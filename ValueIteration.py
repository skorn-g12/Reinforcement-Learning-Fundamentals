...
def EvaluatePolicyGreedy(policy, grid):
    thr = 1e-5
    iter = 0
    while True:
        delta = 0
        for s in states:  # For every state
            ...
            if s in terminal_states:
                continue
            maxThisGuy = []
            for a in actions[s]:  # For the action specified in the policy
                grid.set_state(s)
                ...
                val = (r + gamma * V[s2])
                maxThisGuy.append(val)
            # v_new_idx = np.argmax(maxThisGuy)
            v_new = max(maxThisGuy)
            V[s] = v_new
...
def ImprovePolicy(grid):
    for s in states:
        ...
        for a in actions[s]:
            ...
            si, sj, r = grid.move(s, a)
            s2 = (si, sj)
            maxThisGuy.append((r + gamma * V[s2]))
...
if __name__ == "__main__":
    ...
    EvaluatePolicyGreedy(policy, grid)
    ImprovePolicy(grid)
    # print(iter)
